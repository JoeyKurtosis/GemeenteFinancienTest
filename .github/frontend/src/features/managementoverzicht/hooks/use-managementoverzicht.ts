...
import { fetchManagementoverzicht, type Managementoverzicht } from "../api";
 ...
 */
export function useManagementoverzicht() {
    const { applied, isReady } = useFilters();

    const [data, setData] = useState<Managementoverzicht | null>(null);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setIsLoading(true);
            try {
                const next = await fetchManagementoverzicht({
                    jaar,
                    verslagsoort,
                    gemeente,
                    referentie,
    ...
    }, [isReady, jaar, verslagsoort, gemeente, referentie, reservemutaties]);
