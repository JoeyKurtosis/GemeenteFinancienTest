...
export const Button: {
    ...
    if ("href" in commonProps) {
        // Strip React Aria-only props; the anchor/TanStack Link don't accept them.
        // The remaining props are cast loosely because `commonProps` still carries
        // the wide React Aria link prop type that isn't assignable to a DOM element.
        const { isDisabled: _isDisabled, href: _href, ...rest } = commonProps;
        const linkProps = rest as Record<string, unknown>;
        const isExternal = typeof href === "string" && /^(https?:|mailto:|tel:)/.test(href);

        // Disabled links render as a non-navigable element.
        if (disabled || !href) {
            return <span {...linkProps} aria-disabled />;
        }

        // External links can't use the router; fall back to a native anchor.
        if (isExternal) {
