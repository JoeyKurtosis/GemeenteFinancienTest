...
const viewOptions: ViewOption[] = [
  { value: 'day', label: 'Dag', addon: '⌘D' },
  { value: 'week', label: 'Week', addon: '⌘W' },
  { value: 'month', label: 'Maand', addon: '⌘M' },
]
...
const MonthView = ({
  ...
  return (
    <div className={cx('flex flex-1 flex-col', className)}>
          ...
          return (
            ...
            >
              ...
              {remainingEventsCount > 0 && (
                <div className="text-utility-neutral-500 truncate text-xs font-semibold max-md:pl-1">{`${remainingEventsCount} meer…`}</div>
              )}
            </CalendarMonthViewCell>
          )
        })}
      ...
      {targetDateForFooter && (
        <div className="border-secondary border-t px-4 py-5 md:hidden">
          ...
          {eventsForFooter.length > 0 && (
            <div className="mt-4 flex flex-col gap-1.5">
              {eventsForFooter.slice(0, 3).map((event) => {
                ...
                const supportingText = isAllDay ? 'Hele dag' : formatTimeForMonth(event.start)
          ...
          {eventsForFooter.length === 0 && (
            <p className="text-quaternary mt-4 text-xs font-semibold">Geen items voor deze dag.</p>
          )}
          {eventsForFooter.length > 3 && (
            <p className="text-quaternary mt-4 text-xs font-semibold">{`${eventsForFooter.length - 3} meer…`}</p>
          )}
        </div>
      )}
    </div>
...
const WeekViewDay = ({
  ...
  return (
    <div className="border-secondary flex flex-col">
      <div className="bg-primary relative flex-1" style={{ minHeight: `${48 * slotHeight}px` }}>
        {Array.from({ length: 48 }).map((_, slotIndex) => (
          <CalendarDwViewCell
...
const WeekView = ({
  ...
  if (showTimeMarker) {
    ...
    const minutesFromTop =
      localCurrentTime.hour * 60 + localCurrentTime.minute - (startOfVisibleDay.hour * 60 + startOfVisibleDay.minute)
  ...
  return (
    <div className={cx('flex flex-1 flex-col overflow-auto', className)}>
      <div className="bg-primary dark:border-secondary sticky top-0 z-10 grid grid-cols-7 pl-18 shadow-sm dark:border-b">
        {days.map((day) => {
      ...
      <div ref={scrollContainerRef} className="relative flex flex-1 overflow-y-auto">
        {/* Time Gutter */}
        <div className="border-secondary flex h-max w-18 flex-col border-r">
          {hours.map((hour) => {
        ...
        {showTimeMarker && (
          <CalendarTimeMarker style={{ top: `${timeMarkerTop}px` }}>{formatTime(localCurrentTime)}</CalendarTimeMarker>
        )}
      </div>
    </div>
  )
...
const DayView = ({
  ...
  return (
    <div className={cx('flex flex-1 flex-col overflow-auto', className)}>
      <div className="bg-primary dark:border-secondary sticky top-0 z-20 grid grid-cols-7 shadow-sm lg:hidden dark:border-b">
        {days.map((day) => {
          const isTodayFlag = isDateToday(day, timeZone)
          const dayToHighlight =
            selectedDate && day.compare(currentWeekStart) >= 0 && day.compare(currentWeekEnd) <= 0
      ...
      <div className="flex h-full flex-1">
        <div ref={scrollContainerRef} className="relative flex h-full flex-1 overflow-auto">
          <div className="border-secondary flex h-max w-14 flex-col border-r md:w-18">
            {hours.map((hour) => {
        ...
        <div className="border-secondary sticky top-0 hidden h-full w-82 flex-col overflow-auto border-l mask-b-from-94% lg:flex">
          <AriaCalendar
            aria-label="Calendar"
            className="px-6 py-5"
            value={selectedDate}
          ...
          <div className="border-secondary flex flex-col gap-5 border-t px-6 py-5">
            <div className="flex flex-col gap-2">
              <section className="flex w-full justify-between">
                <p className="text-md text-primary font-semibold">Product demo</p>
              ...
              <section className="flex flex-col gap-1.5">
                <div className="flex items-center gap-1.5">
                  <CalendarIcon className="text-fg-quaternary size-4 stroke-[2.25px]" />
                  <p className="text-tertiary text-sm">Friday, Jan 10, 2027</p>
                </div>
                <div className="flex items-center gap-1.5">
                  <Clock className="text-fg-quaternary size-4 stroke-[2.25px]" />
                  <p className="text-tertiary text-sm">1:30 PM - 3:30 PM</p>
                </div>
                <div className="flex items-center gap-1.5">
                  <BellRinging01 className="text-fg-quaternary size-4 stroke-[2.25px]" />
                  <p className="text-tertiary text-sm">10 min before</p>
                </div>
              </section>
            </div>
              ...
              <section className="flex gap-2">
                <section className="flex flex-row -space-x-2">
                  ...
                  <Avatar
                    ...
                    size="sm"
                  />
                  <Avatar className="ring-bg-primary ring-[1.5px]" initials="OR" size="sm" />
                </section>
                <AvatarAddButton size="sm" />
              </section>

              <section className="flex items-center gap-2">
                <p className="text-primary text-sm font-semibold">6 guests</p>
                <span className="border-primary h-[13px] border-l" />
                <p className="text-tertiary text-sm">5 yes</p>
                <span className="border-primary h-[13px] border-l" />
                <p className="text-tertiary text-sm">1 awaiting</p>
              </section>
            </div>

            <section className="flex flex-col gap-2">
              <p className="text-primary text-sm font-semibold">About this event</p>
...
export const Calendar = ({ events, view: defaultView = 'month', className }: CalendarProps) => {
  ...
  return (
    <div
      ...
      aria-label="Calendar"
      className={cx(
        'bg-primary ring-secondary flex flex-col overflow-hidden rounded-xl shadow-xs ring',
