...
const Connector = ({ type, size }: { type: 'line' | 'end' | 'none'; size: 'sm' | 'md' }) => (
  <div className="relative size-4 shrink-0" data-tree-connector>
    {type === 'line' && (
      <div
        className={cx(
          'bg-border-secondary absolute top-1/2 left-1/2 w-px -translate-x-1/2 -translate-y-1/2',
          size === 'sm' ? 'h-[42px]' : 'h-[44px]'
        )}
      />
    )}
    {type === 'end' && (
      <div className={cx('absolute bottom-2 left-1/2 w-px -translate-x-1/2', size === 'sm' ? 'h-[22px]' : 'h-[24px]')}>
        <div
          className={cx(
            'border-border-secondary border-b border-l',
...
const TreeViewDropIndicator = (props: { target: Parameters<typeof AriaDropIndicator>[0]['target'] }) => (
  <AriaDropIndicator
    {...props}
    className="before:bg-brand-solid relative h-0 outline-hidden before:absolute before:inset-x-0 before:-top-px before:z-10 before:h-0.5 before:rounded-full"
  />
)
...
const TreeViewRoot = <T extends object>({
   ...
   */
  const prevKeysRef = useRef<Set<Key>>(
    defaultSelectedKeys && defaultSelectedKeys !== 'all' ? new Set<Key>(defaultSelectedKeys) : new Set<Key>()
  )
  ...
  const contextValue = useMemo<TreeViewContextType>(
    () => ({ size, showConnectors, selectedKeys, collectionRef }),
    [size, showConnectors, selectedKeys]
  )
...
const TreeViewItem = ({ className, ...props }: TreeViewItemProps) => (
  <AriaTreeItem
    {...props}
    className={(state) =>
      cx('mt-0.5 outline-hidden first:mt-0', typeof className === 'function' ? className(state) : className)
    }
  />
)
      ...
      {(state) => {
        ...
        if (showConnectors && itemKey != null && level > 1) {
          ...
          for (let depth = 0; depth < level - 1; depth++) {
            ...
            connectors.unshift(
              <Connector key={depth} size={size} type={isLast ? (depth === 0 ? 'end' : 'none') : 'line'} />
            )
        ...
        return (
          <div
            className={cx(
              'outline-focus-ring flex w-full cursor-pointer items-center gap-2 rounded-sm -outline-offset-2 transition duration-100 ease-linear',
            ...
            <div className={cx('flex min-w-0 flex-1 items-center', size === 'sm' ? 'gap-1.5' : 'gap-2')}>
              {Icon &&
                (isReactComponent(Icon) ? (
                  <Icon className={cx('text-fg-quaternary shrink-0', size === 'sm' ? 'size-4' : 'size-5')} />
                ) : isValidElement(Icon) ? (
                  Icon
                ) : null)}

              <span className="text-tertiary min-w-0 flex-1 truncate text-sm font-semibold">{children}</span>

              {action}

              {allowsDragging && (
                <AriaButton slot="drag" className={cx('shrink-0 cursor-grab outline-hidden', !isSelected && 'hidden')}>
                  <DotsGrid className="text-fg-quaternary size-4" />
                </AriaButton>
              )}
            </div>
          </div>
