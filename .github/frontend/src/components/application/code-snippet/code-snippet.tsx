...
const CodeSnippetComponent = ({
  ...
  const codeBlock = (
    <div
      {...otherProps}
      className={cx(
        'group/code-snippet bg-primary relative max-w-full overflow-clip rounded-xl ring-1',
        isModern ? 'ring-secondary_alt shadow-lg' : 'ring-secondary',
        className
      )}
    >
      {/* Code content */}
  ...
  if (isModern) {
    return <div className="bg-secondary_alt ring-secondary rounded-[20px] p-2 ring-1 ring-inset">{codeBlock}</div>
  }
  ...
  return (
    <TabsRefContext value={rootRef}>
      <AriaTabs
        ...
        className={(values) =>
          cx(
            'group/code-tabs bg-secondary_alt ring-secondary rounded-t-xl rounded-b-[20px] shadow-xs ring-1 ring-inset',
  ...
  return (
    <div className="relative flex items-start">
      <AriaTabList
        ...
        className={(values) =>
          cx(
            'flex flex-1 items-center overflow-auto px-3 pt-2 pb-2',
            typeof className === 'function' ? className(values) : className
          )
        }
      >
...
const CodeSnippetTab = ({ children, className, ...otherProps }: CodeSnippetTabProps) => {
  return (
    <AriaTab
      ...
      className={(state) =>
        cx(
          'text-quaternary outline-focus-ring z-10 flex cursor-pointer items-center justify-center gap-2 rounded-md px-2 py-1 text-sm font-semibold whitespace-nowrap outline-0 outline-offset-2 transition duration-100 ease-linear',
...
const CodeSnippetTabPanel = ({ children, className, ...otherProps }: TabPanelProps) => {
  return (
    <AriaTabPanel
      {...otherProps}
      className={(state) => cx('px-2 pb-2', typeof className === 'function' ? className(state) : className)}
    >
      {children}
    </AriaTabPanel>
  )
