...
import { ImageProvider, useImageColorPicker } from './image-picker-context'
...
const AdjustmentSlider = ({ label, minValue = -100, maxValue = 100, ...props }: AdjustmentSliderProps) => (
  <AriaSlider
    ...
    maxValue={maxValue}
    className={(state) =>
      cx('flex items-center gap-3', typeof props.className === 'function' ? props.className(state) : props.className)
    }
  >
    <span className="text-secondary w-[88px] shrink-0 text-sm font-semibold">{label}</span>
    <AriaSliderTrack className="border-primary bg-tertiary relative h-3 flex-1 cursor-pointer rounded-full border">
      <AriaSliderThumb
        className={({ isFocusVisible, isDragging }) =>
          cx(
            'bg-fg-white ring-secondary_alt outline-focus-ring top-1/2 size-4 cursor-grab rounded-full shadow-md ring outline-0',
