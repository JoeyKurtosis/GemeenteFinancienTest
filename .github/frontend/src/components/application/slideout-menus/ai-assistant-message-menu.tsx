...
const GoogleCalendarIcon = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" {...props}>
      ...
      />
      <path
        d="M13.6881 14.2321V10.8104L12.9026 11.1397L12.5911 10.423L13.8958 9.81619H14.5384V14.2321H13.6881Z"
        fill="#4285F4"
      />
    </svg>
  )
...
export const AIAssistantMessageMenu = () => {
  ...
  return (
    <SlideoutMenu isOpen={isOpen} onOpenChange={setIsOpen} isDismissable className="overflow-auto">
      <SlideoutMenu.Header className="bg-primary/70 sticky top-0 right-0 left-0 px-4 pt-6 pb-8 backdrop-blur-sm md:px-6">
        <Logo />
      </SlideoutMenu.Header>
          ...
          {messages.map((msg) => {
            ...
            if (msg.meetings) {
              return (
                <li
                  ...
                  className={cx(
                    'peer bg-primary ring-secondary flex flex-col gap-3 rounded-lg p-3 ring-1 ring-inset',
                    marginTop
                  )}
                >
                  {msg.meetings.map((meeting) => (
                    <div key={meeting.id} className="flex gap-2">
                      <div className="ring-secondary size-6 rounded-md ring-1 ring-inset">
                        <GoogleCalendarIcon className="shrink-0" />
                      </div>

                      <p className="text-tertiary mt-0.5 text-sm">
                        <span className="text-primary font-semibold underline underline-offset-2">{meeting.title}</span>{' '}
                        {meeting.description}.
                      </p>
                    </div>
                  ))}
            ...
            return (
              <MessageItem
                ...
                data-self={msg.user?.me}
                msg={msg}
                className={cx('peer', marginTop)}
                showUserLabel={false}
              />
            )
          })}
        </ol>
        <div className="bg-primary sticky bottom-0 mt-auto flex flex-col px-4 pb-4 md:px-6 md:pb-5">
