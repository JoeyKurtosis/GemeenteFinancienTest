...
export const SectionLabelRoot = ({
  ...
  return (
    <div className={className}>
      <h3 className={cx('text-secondary flex items-center', styles[size].heading)}>
        {title}

        <span className={cx('text-brand-tertiary hidden', isRequired && 'block')}>*</span>

        {(tooltip || tooltipDescription) && (
          <Tooltip title={tooltip} description={tooltipDescription}>
            <TooltipTrigger className="text-fg-quaternary hover:text-fg-quaternary_hover focus:text-fg-quaternary_hover transition duration-200">
