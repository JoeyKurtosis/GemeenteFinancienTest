  ...
  return (
    ...
    >
      {(state) => (
        <div
          className={cx(
            'outline-focus-ring hover:bg-primary_hover relative flex min-h-10 items-center justify-between rounded-lg px-2.5 py-2 pl-2.5 transition duration-100 ease-linear',
            state.isSelected && 'bg-primary_hover',
            state.isFocusVisible && 'outline-2 outline-offset-2',
            stacked && 'items-start p-2.5 pl-3.5'
          )}
        >
          {props.type === 'icon' && !stacked && <props.icon className="text-fg-quaternary mr-2 size-5" />}
          {props.type === 'icon' && stacked && (
            <FeaturedIcon color="gray" size="md" theme="modern" icon={props.icon} className="mr-2" />
          )}

          {props.type === 'avatar' && (
            <Avatar alt={props.alt} src={props.src} size={stacked ? 'md' : 'xs'} className="mr-2" />
          )}
          ...
          {props.type === 'dot' && (
            <svg
              ...
              viewBox="0 0 10 10"
              fill="none"
              className={cx('text-fg-success-secondary mr-2 shrink-0', stacked && 'mt-2 self-start')}
            >
          ...
          <div className={cx('flex flex-1 gap-x-1.5', stacked && 'flex-col')}>
            <AriaText slot="label" className="text-primary text-sm font-medium">
