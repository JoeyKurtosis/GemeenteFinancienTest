    ...
    return (
        <Dialog>
            ...
            <DialogContent className="aui-attachment-preview-dialog-content p-2 sm:max-w-3xl [&_svg]:text-background [&>button]:rounded-full [&>button]:bg-foreground/60 [&>button]:p-1 [&>button]:opacity-100 [&>button]:ring-0! [&>button]:hover:[&_svg]:text-destructive">
                <DialogTitle className="aui-sr-only sr-only">Voorbeeld afbeeldingbijlage</DialogTitle>
    ...
    return (
        <Tooltip>
            <AttachmentPrimitive.Root
                className={cn("aui-attachment-root relative", isImage && !isComposer && "aui-attachment-root-message only:*:first:size-24")}
            >
                <AttachmentPreviewDialog>
                    <TooltipTrigger asChild>
                        <div
                            ...
                            role="button"
                            tabIndex={0}
                            aria-label={`${typeLabel} bijlage${isError ? ", uploaden mislukt" : isUploading ? ", bezig met uploaden" : ""}`}
                        >
                            <AttachmentThumb />
                            {isUploading && (
                                <div
    ...
    return (
        <ComposerPrimitive.AddAttachment asChild>
            <TooltipIconButton
                ...
                variant="ghost"
                size="icon"
                className="aui-composer-add-attachment size-7 rounded-full bg-brand-500 p-1 text-xs font-semibold text-white hover:bg-brand-600 dark:border-muted-foreground/15 dark:hover:bg-muted-foreground/30"
