...
const ColorThumb = ({ className, ...props }: AriaColorThumbProps) => (
  <AriaColorThumb
    className={(state) =>
      cx(
        'border-fg-white outline-focus-ring size-5 cursor-grab rounded-full border-[3px] shadow-md outline-0 focus-visible:outline-2 focus-visible:outline-offset-2',
...
const ColorPickerArea = ({ style, className, ...props }: AriaColorAreaProps) => (
  <AriaColorArea
    ...
    className={(state) =>
      cx(
        'ring-alpha-black/10 relative aspect-square w-full shrink-0 rounded-lg ring-[0.5px] ring-inset',
        typeof className === 'function' ? className(state) : className
      )
    }
  >
...
const HueSlider = ({ className, ...props }: Omit<AriaColorSliderProps, 'channel'>) => (
  ...
  >
    <AriaSliderTrack
      ...
      className="ring-alpha-black/10 relative h-3 w-full cursor-pointer rounded-full ring-[0.5px] ring-inset"
    >
...
export const InputCell = ({
  ...
  return (
    <div
      className={cx(
        'bg-primary ring-primary focus-within:ring-brand relative flex place-content-center place-items-center ring-1 transition-shadow duration-100 ease-linear ring-inset focus-within:z-10 focus-within:ring-2',
        radius,
...
export const SwatchItem = ({ color, size = 'md', ...props }: SwatchItemsProps) => (
  ...
  >
    <AriaColorSwatch
      className="ring-alpha-black/10 size-full rounded-full ring-1 ring-inset"
  ...
  return (
    <div className="flex flex-1 shadow-xs">
      <InputCell position="first" className="flex-1">
        ...
        >
          <AriaColorSwatch
            className="ring-alpha-black/10 size-4 shrink-0 rounded-full ring-1 ring-inset"
            style={({ defaultStyle }) => defaultStyle}
          />
          <AriaInput
            onFocus={selectAllOnFocus}
            className="text-primary w-full min-w-0 bg-transparent text-sm outline-hidden"
          />
        </AriaColorField>
      </InputCell>
      <InputCell position="last" className="w-14 shrink-0">
        ...
        >
          <AriaInput
            onFocus={selectAllOnFocus}
            className="text-primary w-full min-w-0 bg-transparent px-2.5 py-2 text-sm outline-hidden"
          />
        </AriaColorField>
      </InputCell>
    </div>
  ...
  return (
    <InputCell className="flex-1 shadow-xs">
      <input
        ...
        className="text-primary w-full min-w-0 bg-transparent px-3 py-2 text-sm outline-hidden"
      />
    </InputCell>
  )
}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-3">
        <p className="text-secondary flex-1 text-sm font-semibold">{label}</p>
        {onAdd && (
        ...
        <div className="flex flex-wrap gap-2">
          {colors.map((c, i) => (
            <button
              ...
              type="button"
              onClick={() => onSelect?.(c)}
              className="ring-alpha-black/10 focus-visible:outline-focus-ring size-5 cursor-pointer rounded-full ring-1 transition duration-100 ease-linear ring-inset hover:scale-110 focus-visible:outline-2 focus-visible:outline-offset-2"
              style={{ background: c }}
...
const ColorPickerDialog = ({ className, children }: { className?: string; children?: ReactNode }) => (
  <div
    className={cx(
      'bg-primary ring-secondary_alt relative flex w-80 flex-col overflow-clip rounded-2xl shadow-xl ring-1',
      className
    )}
  >
    {children}
