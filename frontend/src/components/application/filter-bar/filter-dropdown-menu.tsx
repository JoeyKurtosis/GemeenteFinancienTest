'use client'

import { Button } from '@/components/base/buttons/button'
import { cx } from '@/utils/cx'
import { ChevronDown, FilterLines } from '@untitledui/icons'
import type { ReactNode } from 'react'
import { Dialog as AriaDialog, DialogTrigger as AriaDialogTrigger, Popover as AriaPopover } from 'react-aria-components'

export interface FilterRow {
  id: string
  field: string
  operator: string
  value: string
}

export interface FilterDialogProps {
  /** The filter rows currently in the dialog */
  filters: FilterRow[]
  /** Called when "Apply filter" is clicked */
  onApply?: (filters: FilterRow[]) => void
  /** Called when "Clear all" is clicked */
  onClearAll?: () => void
  /** Called when a filter row is removed */
  onRemoveFilter?: (id: string) => void
  /** Called when a filter row changes */
  onFilterChange?: (id: string, patch: Partial<Omit<FilterRow, 'id'>>) => void
  /** Render function for a single filter row's inputs */
  renderFilterRow: (filter: FilterRow, onChange: (patch: Partial<Omit<FilterRow, 'id'>>) => void) => ReactNode
  /** Additional class name */
  className?: string
}

const FilterRowItem = ({ children, onRemove }: { children: ReactNode; onRemove?: () => void }) => (
  <div className="flex items-start gap-1">
    <div className="flex min-w-0 flex-1 items-center gap-3">{children}</div>
  </div>
)

export const FilterDialog = ({
  filters,
  onApply,
  onClearAll,
  onRemoveFilter,
  onFilterChange,
  renderFilterRow,
  className,
}: FilterDialogProps) => {
  const hasFilters = filters.length > 0

  return (
    <AriaDialog
      className={cx(
        'bg-primary ring-secondary_alt overflow-hidden rounded-lg shadow-lg ring-1 outline-hidden',
        className
      )}
    >
      {({ close }) =>
        !hasFilters ? (
          <div className="flex flex-col gap-3 p-4">
            <div className="flex flex-col gap-1 text-sm">
              <p className="text-primary font-semibold">No filters applied</p>
            </div>
          </div>
        ) : (
          <>
            <div className="flex flex-col gap-3 p-4">
              <div className="grid grid-cols-3 gap-x-3 gap-y-3">
                {filters.map((filter) => (
                  <FilterRowItem key={filter.id} onRemove={() => onRemoveFilter?.(filter.id)}>
                    {renderFilterRow(filter, (patch) => onFilterChange?.(filter.id, patch))}
                  </FilterRowItem>
                ))}
              </div>
            </div>

            <div className="border-secondary flex items-center justify-end border-t px-4 py-3">
              <div className="flex items-center gap-3">
                <Button size="xs" color="secondary" onClick={() => onClearAll?.()}>
                  Reset filter
                </Button>
                <Button
                  size="xs"
                  color="primary"
                  onClick={() => {
                    onApply?.(filters)
                    close()
                  }}
                >
                  Filter toepassen
                </Button>
              </div>
            </div>
          </>
        )
      }
    </AriaDialog>
  )
}

export interface FilterDropdownProps extends FilterDialogProps {
  /** Number of applied (committed) filters to show in the badge. Only used by the default trigger. */
  appliedCount?: number
  /** Whether the trigger is disabled. Only used by the default trigger. */
  isDisabled?: boolean
  /** Popover placement relative to trigger */
  placement?: 'bottom' | 'bottom start' | 'bottom end'
  /** Custom trigger element. When provided, replaces the default "Filters" button. */
  children?: ReactNode
}

export const CountBadge = ({ count, className }: { count: number; className?: string }) => (
  <span
    className={cx(
      'border-primary bg-primary text-secondary inline-flex items-center rounded-md border px-1.5 py-0.5 text-xs leading-[18px] font-medium shadow-xs',
      className
    )}
  >
    {count}
  </span>
)

export const FilterDropdown = ({
  filters,
  appliedCount,
  onApply,
  onClearAll,
  onRemoveFilter,
  onFilterChange,
  renderFilterRow,
  isDisabled,
  className,
  placement = 'bottom end',
  children,
}: FilterDropdownProps) => {
  const hasFilters = filters.length > 0
  const hasApplied = appliedCount != null && appliedCount > 0

  return (
    <AriaDialogTrigger>
      {children ?? (
        <Button
          color="secondary"
          size="sm"
          iconLeading={FilterLines}
          iconTrailing={ChevronDown}
          isDisabled={isDisabled}
          className={cx('max-h-9', hasApplied && 'bg-primary_hover', className)}
        >
          <span className="flex items-center gap-1.5">
            Filters
            {hasApplied && <CountBadge count={appliedCount} />}
          </span>
        </Button>
      )}
      <AriaPopover
        placement={placement}
        offset={4}
        containerPadding={0}
        className={(state) =>
          cx(
            'origin-(--trigger-anchor-point) will-change-transform md:w-[624px]',
            state.isEntering &&
              'animate-in fade-in placement-top:slide-in-from-bottom-0.5 placement-bottom:slide-in-from-top-0.5 duration-150 ease-out',
            state.isExiting &&
              'animate-out fade-out placement-top:slide-out-to-bottom-0.5 placement-bottom:slide-out-to-top-0.5 duration-100 ease-in',
            !hasFilters && 'w-[280px]'
          )
        }
      >
        <FilterDialog
          filters={filters}
          onApply={onApply}
          onClearAll={onClearAll}
          onRemoveFilter={onRemoveFilter}
          onFilterChange={onFilterChange}
          renderFilterRow={renderFilterRow}
        />
      </AriaPopover>
    </AriaDialogTrigger>
  )
}
