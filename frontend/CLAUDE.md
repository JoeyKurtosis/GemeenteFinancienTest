...
The Gemeentefinanciën dashboard provides insight into the income and expenditure of local government. It also contains information about municipal taxes, debts, and housing costs.

This is the **Gemeentefinanciën** frontend built with:

- **React 19** with TypeScript
- **TanStack Router** for file-based routing with route context
...
│   ├── _layout.tsx        # App layout (sidebar + page header)
│   ├── _layout/           # Routes nested under layout
│   │   └── index.tsx      # Dashboard (/)
...
│   │   ├── components/    # LoginRouteView
│   │   └── index.ts       # Public exports
│   └── dashboard/         # Dashboard feature
│       ├── components/    # DashboardRouteView
│       └── index.ts
├── components/
│   ├── layout/            # App layout components (PageHeader, AppSidebar)
...
├── providers/             # React context providers (ThemeProvider, RouteProvider)
...
```
features/<name>/
├── api.ts                 # Fetch functions (GET, POST, PATCH, DELETE)
├── keys.ts                # React Query key factory (when using TanStack Query)
├── schema.ts              # Zod validation schemas
├── index.ts               # Public exports
├── context/               # Context providers (if needed)
│   └── <feature>-context.tsx
├── components/
│   ├── <feature>-route-view.tsx  # Main page component
│   └── (sub-components)
└── hooks/
    └── use-<entity>-query.ts
```
...
Uses **TanStack Router** with file-based routing. Routes are in `src/routes/`.

**Route context** is used to pass metadata (title, actions, breadcrumbs) to the layout:
```typescript
export const Route = createFileRoute("/_layout/example")({
    component: ExampleRouteView,
    context: () => ({
        title: "Example Page",
        showBreadCrumbs: true,
        actions: [
            { label: "Nieuwe", icon: Plus, href: "/example/new" },
        ],
    }),
});
```

The `useRouteMetadata()` hook extracts this context in the layout to render the `PageHeader`.

### Authentication

- `AuthProvider` wraps the app, calls `/api/auth/me/` on mount
- `useAuth()` hook provides: `user`, `isAuthenticated`, `isLoading`, `login`, `logout`
- Dashboard is **public** (no auth required)
- Login is available via profile card in sidebar or `/login` route
- Backend auth endpoints: `/api/auth/login/`, `/api/auth/logout/`, `/api/auth/me/`
